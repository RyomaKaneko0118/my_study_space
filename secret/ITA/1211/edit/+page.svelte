<script lang="ts">
  ...
  $: hasChanged =
    ...
    $form.createLecturerEmergencyContactInputs.length !== 0 ||
    ...
    $form.updateLecturerEmergencyContactInputs.some(
      (updateLecturerEmergencyContactInput) => {
        const lecturerEmergencyContact =
          $lecturer.lecturerEmergencyContacts.find(
            (lecturerEmergencyContact) =>
              lecturerEmergencyContact.id ===
              updateLecturerEmergencyContactInput.id
          )
        return (
          lecturerEmergencyContact &&
          (updateLecturerEmergencyContactInput.name !==
            lecturerEmergencyContact.name ||
            updateLecturerEmergencyContactInput.relationship !==
              lecturerEmergencyContact.relationship ||
            updateLecturerEmergencyContactInput.phoneNumber !==
              lecturerEmergencyContact.phoneNumber)
        )
      }
    ) ||
    deleteLecturerContactIds.length !== 0 ||
    deleteLecturerEmergencyContactIds.length !== 0 ||
  ...
  let deleteLecturerEmergencyContactIds: number[] = []
  ...
  const deleteCurrentEmergencyContact = (id: number) => {
    resetError()
    $form.updateLecturerEmergencyContactInputs =
      $form.updateLecturerEmergencyContactInputs.filter(
        (updateLecturerEmergencyContactInput) =>
          updateLecturerEmergencyContactInput.id !== id
      )
  ...
  const { form, errors, handleSubmit } = createForm({
    initialValues: {
      ...
      updateLecturerEmergencyContactInputs:
        $lecturer.lecturerEmergencyContacts.map((lecturerEmergencyContact) => {
          return {
            id: lecturerEmergencyContact.id,
            name: lecturerEmergencyContact.name,
            relationship: lecturerEmergencyContact.relationship,
            phoneNumber: lecturerEmergencyContact.phoneNumber
          }
        })
    },
    validationSchema: yup.object().shape({
      ...
      updateLecturerEmergencyContactInputs: yup.array().of(
        yup.object().shape({
          name: yup.string().required(),
          relationship: yup.string().required(),
          phoneNumber: yup.string().required()
    ...
    onSubmit: async (values) => {
      ...
      if (
        input.masterLecturerAffiliatedOrganizationId === null &&
        $lecturer.lecturerContract?.contractorType === 'ORGANIZATION'
      ) {
        errors.set({
          ...$errors,
          masterLecturerAffiliatedOrganizationId:
            '契約の名義が"組織"に設定されているため"所属組織なし"に変更することはできません\n契約の名義を"個人"に変更してください'
        })
        return
      }
  ...
  <div class="card my-md py-lg px-md">
    <form class="form" on:submit={handleSubmit}>
      ...
      <div class="input-group mt-md">
        ...
        <div class="w-100 d-flex flex-direction-column">
          <div>
            {#each $form.updateLecturerEmergencyContactInputs as updateLecturerEmergencyContactInput, i}
              {@const error = emergencyContactErrorFormatter(
                $errors.updateLecturerEmergencyContactInputs[i]
              )}
              <div class="mb-sm">
                <div class="d-flex">
                  <CommonInput
                    ...
                    placeholder="氏名"
                    invalid={!!error?.name}
                    bind:value={updateLecturerEmergencyContactInput.name}
                    on:input={resetError}
                  />
                  <div class="delete-btn-wrapper ml-md text-error">
                    <button
                ...
                <div class="d-flex mt-xs">
                  <div>
                    <CommonInput
                      ...
                      bind:value={updateLecturerEmergencyContactInput.relationship}
                      on:input={resetError}
                    />
                  </div>
                  <CommonInput
                    ...
                    bind:value={updateLecturerEmergencyContactInput.phoneNumber}
                    on:input={resetError}
                  />
                </div>
                {#if error}
