<script lang="ts">
  ...
  import {
    ...
    type CreateMasterLecturerAffiliatedOrganizationInput
  ...
  const { form, errors, handleSubmit } = createForm({
    initialValues: {
      ...
      corporateNumber: '',
      invoiceNumber: '',
      equityStock: undefined as undefined | number,
      link: '',
      phoneNumber: '',
      postalCode: '',
    ...
    onSubmit: async (values) => {
      const input: CreateMasterLecturerAffiliatedOrganizationInput = {
        ...values,
        corporateNumber: values.corporateNumber || undefined,
        invoiceNumber: values.invoiceNumber || undefined,
        equityStock: values.equityStock ? Number(values.equityStock) : undefined
      }
      ...
      try {
        const result = await apolloClient.mutate({
          mutation:
            masterLecturerAffiliatedOrganizationsMutations.createMasterLecturerAffiliatedOrganization,
          variables: {
            createMasterLecturerAffiliatedOrganizationInput: input
          }
        })
        masterLecturerAffiliatedOrganization.set(
          new MasterLecturerAffiliatedOrganization(
            result.data.createMasterLecturerAffiliatedOrganization
          )
        )
        showCreatedModalWithId =
          result.data.createMasterLecturerAffiliatedOrganization.id
      } catch (e) {
...
<svelte:head>
  <title>所属組織登録 | {PUBLIC_TITLE}</title>
</svelte:head>

<section class="content">
  <h1 class="text-primary">
    <Fa icon={faUserGroup} />
    <span class="ml-sm">所属組織登録</span>
  </h1>
...
<CommonModal
  ...
  message="所属組織を登録しました。"
  on:close={() =>
    goto(`/master-lecturer-affiliated-organizations/${showCreatedModalWithId}`)}
/>
