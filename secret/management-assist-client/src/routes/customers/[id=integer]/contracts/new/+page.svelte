<script lang="ts">
  ...
  const { form, errors, handleSubmit } = createForm({
    initialValues: {
      ...
      autoUpdateCountOfMonths: undefined as undefined | number,
      _autoUpdate: false
    },
    ...
    onSubmit: async (values) => {
      ...
      const input: CreateContractInput = {
        ...
        ...attr,
        startDate: new Date(attr.startDate as string),
        endDate: attr.endDate ? new Date(attr.endDate) : undefined,
      ...
      try {
        const result = await apolloClient.mutate({
          mutation: contractsMutations.createContract,
          variables: {
            createContractInput: input
          }
        })
        showCreatedModalWithId = result.data.createContract.id
      } catch {
...
<CommonModal
  ...
  message="契約を登録しました。"
  on:close={() =>
    goto(`/customers/${$customer.id}/contracts/${showCreatedModalWithId}`)}
/>
